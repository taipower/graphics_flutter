...
class ExampleTransformation extends StatefulWidget {
  _MyAppState createState() => _MyAppState();
}

class _MyAppState extends State<ExampleTransformation> {
  ...
  initVertexBuffers(gl) {
    ...
    Utils.loadImage('assets/images/flutter540.jpg').then((bytes) {
      ...
      Utils.loadImage('assets/images/awesomeface.png').then((bytes) {
        ...
        math.Matrix4 trans = math.Matrix4.identity()..rotateZ(80.1)..scale(0.5,0.5,0.5);

        var transPosition = gl.getUniformLocation(glProgram, 'transform');
        if(transPosition < 0){
          print('Failed to get the storage location of transform');
          return -1;
        }
        gl.uniformMatrix4fv(transPosition, true, trans.storage);
      });
    });
