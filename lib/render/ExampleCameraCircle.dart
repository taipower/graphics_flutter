...
class ExampleCameraCircle extends StatefulWidget {
  _MyAppState createState() => _MyAppState();
}

class _MyAppState extends State<ExampleCameraCircle> {
  ...
  render() {
    ...
    double radius = 5.0;
    double camX = sin(current.toDouble() * radius);
    double camZ = cos(current.toDouble() * radius);
    math.Vector3 cameraPosition = math.Vector3(camX, 0.0, camZ);
    math.Vector3 cameraFront = math.Vector3(0.0, 0.0, 0.0);
    math.Vector3 upDirection = math.Vector3(0.0, -1.0, 0.0);
    math.Matrix4 view =
        math.makeViewMatrix(cameraPosition, cameraFront, upDirection);
...
void main() {
  ...
  initVertexBuffers(gl) {
    ...
    gl.vertexAttribPointer(cPosition, 2, gl.FLOAT, false,
        Float32List.bytesPerElement * 5, Float32List.bytesPerElement * 3);
    ...
    Utils.loadImage('assets/images/flutter540.jpg').then((bytes) {
      gl.texImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 540, 540, 0, GL_RGBA,
          GL_UNSIGNED_BYTE, Uint8Array.from(bytes.toList()));
      ...
      Utils.loadImage('assets/images/awesomeface.png').then((bytes) {
        gl.texImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 476, 476, 0, GL_RGBA,
            GL_UNSIGNED_BYTE, Uint8Array.from(bytes.toList()));
        gl.generateMipmap(GL_TEXTURE_2D);
      });
    });
